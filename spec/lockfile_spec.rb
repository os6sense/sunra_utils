...
# require "fakefs" - broken uder 2.1?
require_relative '../sunra_lockfile'
...
describe Sunra::Utils::LockFile do
  ...
  after(:each) { File.delete @lock_file if File.exists? @lock_file }
  ...
  describe :delete do
    ...
    it "provides a #delete method" do
      Sunra::Utils::LockFile.method_defined?(:delete).should eq true
    end
  ...
  describe :create do
    ...
    it "with an array of stings" do
      @lf.create(["1233", "1229"]) 
      expect(File).to exist(@lock_file)
      @lf.contents[0].should eq "1233"
      @lf.contents[1].should eq "1229"
    end
    ...
    it "with a string" do
      @lf.create("hello")
      expect(File).to exist(@lock_file)
      @lf.contents[0].should eq "hello"
    end

    it "with an integer" do
      @lf.create(10)
      expect(File).to exist(@lock_file)
      @lf.contents[0].should eq "10"
    end
  ...
  describe :contents do
    it "provides a #contents method " do
      Sunra::Utils::LockFile.method_defined?(:contents).should eq true
    end

    it "returns an array" do
      @lf.create(["1233", "1229"])
      @lf.contents.is_a?(Array).should eq true
    end
  end
...
describe Sunra::Utils::FFSRelayLockFile do
  ...
  describe :pids do
    it "provides an #pids method" do
      Sunra::Utils::FFSRelayLockFile.method_defined?(:pids).should eq true
    end

    it "returns an array of the pids" do
      @lf.pids.should eq ["1233", "1229"]
    end
  end

  describe :ffserver_pid do
    it "provides an #ffserver_pid method" do
      Sunra::Utils::FFSRelayLockFile.method_defined?(:ffserver_pid).should eq true
    end

    it "returns the pid" do
      @lf.ffserver_pid.should eq "1233"
    end
    ...
    it "is writable" do
      ...
      @lf.capture_pid = "1229"
      @lf.ffserver_pid.should eq "9999"
    end
  end

  describe :capture_pid do
    it "provides an #capture_pid method" do
      Sunra::Utils::FFSRelayLockFile.method_defined?(:capture_pid).should eq true
    end

    it "returns the pid" do
      @lf.capture_pid.should eq "1229"
    end
