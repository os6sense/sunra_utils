...
#
# TODO: Groan, move to Sunra::Utils::PS
module SunraPS
  ...
  #
  # ==== Params
  # +pid+:: pid for which to find children. Must be an integer and less than +MAX_PID+.
  #
  # ==== Returns
  ...
  # ==== Description
  # Determines if a process exists in the list of currently running processed
  # via its pid.
  #
  # ==== Params
  ...
  # +kill_cp+:: Kill child processes (calling it kill_children just seemed *wrong*!)
  # +no_sleep+:: If true no sleep delay will be included. Default false (0.5 seconds)
  #
  # ==== Returns
  # Nothing.
  ...
  # +cmdstring+:: Full command string including both process name and options
  #
  # ==== Returns
  ...
  def run_background_cmd(cmdname, cmdstring)
    ...
    # instance of the command may be running for an entirely unrelated reason
    # then the pid of that process will be incorrectly found. In addition,
    # using spawn provides inaccurate information as to the processes actual
    # pid. This is a major issue if a piped command is required to be run
    # since if the failure of any part of the pipe fails to terminate
    # the other then the process watcher will not be triggered.
    #
    ...
    # processes need to be handled differently with a seperate pipe for each.
    # In any event this is a priority for redesign.
    #
    # NOTE: Be especially careful that the command name is that of the capture
    # process and NOT the conversion pip
    pid = get_pid cmdname
